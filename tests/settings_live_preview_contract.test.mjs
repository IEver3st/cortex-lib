...
assert.match(
  uiSource,
  /function getSettingsUiScale\(\)[\s\S]*?return clamp\(h \/ 1080, 0\.86, 1\)/,
  'settings must never inherit the global high-resolution upscaling above 1x',
)
...
assert.ok(Number(heightMatch[1]) <= 680, 'settings panel desktop height ceiling must be 680px or less')
